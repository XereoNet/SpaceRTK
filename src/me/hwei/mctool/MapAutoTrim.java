...
public class MapAutoTrim {
    ...
    public static void main(String[] args) {
        ...
        if (basePathStr == null) {
            ...
            System.out
                    .println("Usage: java -jar mmat.jar -w <world path> [-d <dilation size>] [-p <user defined preserve block id list (spliter: comma)>]");
            return;
        }
                ...
                if (matcher.find()) {
                    ...
                    for (int x = 0; x < 32; ++x) {
                        for (int z = 0; z < 32; ++z) {
                            ...
                            IntTag xPosTag = (IntTag) levelTag.getValue().get(
                                    "xPos");
                            IntTag zPosTag = (IntTag) levelTag.getValue().get(
                                    "zPos");
                            ...
                            if (toRemain(blocksTag.getValue(), preservedIds)) {
                                ...
                                for (int dx = -DilationSize; dx <= DilationSize; ++dx) {
                                    for (int dz = -DilationSize; dz <= DilationSize; ++dz) {
                                        remainChunkSet.add(new IntPair(cx + dx,
                                                cz + dz));
                                    }
                                }
                            }
